...
export function HarnessPanel({ harnesses, runs, providers, agents, defaultProvider, onLoadModels, onValidate, onCreate, onSave, onDelete, onRun, onAppendRun, onCancelRun, onError }: Props) {
  ...
  const workflowRuns = runs.filter((item) => item.harnessId === selectedId).sort((left, right) => Number(["queued", "running", "waiting"].includes(right.status)) - Number(["queued", "running", "waiting"].includes(left.status)) || right.createdAt.localeCompare(left.createdAt));
