...
type Props = {
  ...
  onResolvePermission?(runId: string, blockId: string, sessionId: string, requestId: string, optionId?: string): Promise<HarnessRun>;
  onAnswerQuestion?(runId: string, blockId: string, sessionId: string, input: string): Promise<HarnessRun>;
  onCancelRun(runId: string): Promise<void>;
  onError(message: string): void;
};

export function HarnessPanel({ harnesses, runs, providers, agents, defaultProvider, onLoadModels, onValidate, onCreate, onSave, onDelete, onRun, onAppendRun, onResolvePermission, onAnswerQuestion, onCancelRun, onError }: Props) {
  ...
  const pausedBlock = run?.blocks.find((item) => item.status === "awaiting_permission" || item.status === "awaiting_user_input");
  ...
  return <div className={`harness-panel ${mode}`}>
    ...
    {!draft ? <div className="harness-empty"><strong>Build an AI workflow</strong><span>Create a workflow, add prompt blocks, then connect their execution order.</span><button onClick={() => setCreateName("New Workflow")}><Plus size={14} /> Create workflow</button></div> : <>
      ...
      {run && pausedBlock && <PauseResolution run={run} block={pausedBlock} label={blockById.get(pausedBlock.blockId)?.label ?? pausedBlock.blockId} onResolvePermission={onResolvePermission} onAnswerQuestion={onAnswerQuestion} onError={onError} />}
...
function PauseResolution({ run, block, label, onResolvePermission, onAnswerQuestion, onError }: { run: HarnessRun; block: HarnessRun["blocks"][number]; label: string; onResolvePermission?: Props["onResolvePermission"]; onAnswerQuestion?: Props["onAnswerQuestion"]; onError(message: string): void }) {
  const [answer, setAnswer] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const owned = Boolean(block.sessionId);
