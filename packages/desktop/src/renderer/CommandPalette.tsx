...
export function CommandPalette({ commands, context, onClose }: { commands: Command[]; context: CommandContext; onClose(): void }) {
  ...
  return (
    <div className="dialog-overlay quick-open-overlay" onMouseDown={onClose}>
      <section className="quick-open-dialog command-palette" role="dialog" aria-modal="true" aria-labelledby="command-palette-title" onMouseDown={(event) => event.stopPropagation()} onKeyDown={onKeyDown}>
        <h2 id="command-palette-title" className="visually-hidden">Command Palette</h2>
        <div className="quick-open-input">
          <Search size={16} aria-hidden="true" />
          <input ref={input} value={query} onChange={(event) => { setQuery(event.target.value); setActiveIndex(0); }} placeholder="Search commands" aria-label="Search commands" role="combobox" aria-expanded="true" aria-controls="command-palette-results" aria-activedescendant={results.length ? `command-${selected}` : undefined} autoComplete="off" spellCheck={false} />
          <kbd>Esc</kbd>
        </div>
        <div id="command-palette-results" className="quick-open-results" role="listbox" aria-label="Commands">
          {results.length === 0 ? <div className="quick-open-empty" role="status">No commands match “{query}”</div> : results.map((command, index) => {
            const enabled = commandEnabled(command, context);
            return (
