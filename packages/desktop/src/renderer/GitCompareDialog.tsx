...
export function GitCompareDialog({ client, reference, label, path, onClose }: Props) {
  ...
  return <div className="floating-window-layer"><section className="git-history-dialog git-compare-dialog" style={{ left: position.x, top: position.y }} role="dialog" aria-modal="true" aria-label="Compare with local">
    ...
    <div className="git-history-content"><aside>{files.length === 0 && !error && <div className="git-log-empty">No differences</div>}{files.map((file) => <button className={selected?.path === file.path ? "selected" : ""} key={`${file.status}:${file.path}`} onClick={() => void selectFile(file)}><FileDiff size={14} /><span><strong>{file.path.split("/").pop()}</strong><small>{file.path}</small></span><time>{file.status}</time></button>)}</aside><main>{error && <div className="git-log-error">{error}</div>}{selected && diff ? <DiffEditor original={diff.originalContent} modified={diff.modifiedContent} language={languageFor(selected.path)} beforeMount={configureMonacoThemes} theme={monacoTheme()} options={{ automaticLayout: true, readOnly: true, renderSideBySide: false, minimap: { enabled: false }, fontSize: 12, scrollBeyondLastLine: false }} /> : <div className="git-log-empty">Select a changed file</div>}</main></div>
  </section></div>;
}
