...
import type { FileRevision } from "@remote-ide/protocol";
...
export function DetachedEditor() {
  ...
  const [content, setContent] = useState(""); const [saved, setSaved] = useState(""); const [revision, setRevision] = useState<FileRevision>(); const [error, setError] = useState(""); const [ready, setReady] = useState(false);
  ...
  useEffect(() => {
    ...
    void client.connect(options.get("host") ?? "127.0.0.1", Number(options.get("port") ?? "7331")).then(async () => {
      ...
      if (type === "useful") {
        const result = await client.request("useful.read", { scope: scope!, name: path });
        if (current) { setContent(result.content); setSaved(result.content); setReady(true); }
      } else {
        const result = await client.request("filesystem.readFile", { path });
        if (current) { setContent(result.content); setSaved(result.content); setRevision(result.revision); setReady(true); }
      }
  ...
  useEffect(() => {
    ...
    const value = content; const timer = setTimeout(() => {
      if (type === "useful") {
        void clientRef.current!.request("useful.write", { scope: scope!, name: path, content: value }).then(() => setSaved((current) => current === content ? current : value)).catch((saveError: unknown) => setError(saveError instanceof Error ? saveError.message : "Autosave failed"));
      } else {
        void clientRef.current!.request("filesystem.writeFile", { path, content: value, expectedRevision: revision }).then((result) => { setRevision(result.revision); setSaved((current) => current === content ? current : value); }).catch((saveError: unknown) => setError(saveError instanceof Error ? saveError.message : "Autosave failed"));
      }
    }, 600);
