  ...
  return <div className="dialog-overlay" onMouseDown={() => !busy && onClose()}>
    <section className="run-config-dialog git-rollback-dialog" role="alertdialog" aria-modal="true" aria-labelledby="git-rewrite-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="git-rollback-content">
        <p>{action === "amend" ? "Only currently staged index changes will be added to the last commit. Unstaged changes stay untouched." : "The commit will be removed with a mixed reset. Its changes return to the working tree unstaged; existing staged changes are also unstaged."}</p>
        ...
        <footer><button type="button" disabled={busy} onClick={onClose}>Cancel</button><button className={risk ? "danger" : ""} autoFocus disabled={busy} onClick={() => onConfirm(risk)}>{busy ? "Rewriting…" : action === "amend" ? "Amend Commit" : "Undo Commit"}</button></footer>
      </div>
    </section>
  </div>;
}
