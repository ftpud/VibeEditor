...
import type { AgentFile, AiConfiguration, AiContentBlock, AiModel, AiProvider, AiProviderDescriptor, AiSession, AiUsage } from "@remote-ide/protocol";
...
export function AiPanel({ provider, providers, session, sessions, models, usage, agents, selectedAgent, attachments, onProviderChange, onAgentChange, onConfigurationChange, onAttachmentsChange, onSend, onSendAsTask, onSteer, onInterrupt, onNewSession, onSwitchSession, onRemoveSession, onResolvePermission }: { provider: AiProvider; providers: AiProviderDescriptor[]; session: AiSession; sessions: AiSession[]; models: AiModel[]; usage?: AiUsage; agents: AgentFile[]; selectedAgent: string; attachments: AiAttachment[]; onProviderChange(provider: AiProvider): void; onAgentChange(agent: string): void; onConfigurationChange(configuration: AiConfiguration): void; onAttachmentsChange(attachments: AiAttachment[]): void; onSend(prompt: string, configuration: AiConfiguration, attachments: AiAttachment[]): Promise<void>; onSendAsTask?: (prompt: string, configuration: AiConfiguration, attachments: AiAttachment[]) => Promise<void>; onSteer(prompt: string): Promise<void>; onInterrupt(): void; onNewSession(): void; onSwitchSession(session: AiSession): void; onRemoveSession(session: AiSession): void; onResolvePermission(requestId: string, optionId?: string): void }) {
  ...
  }, [session.id, session.messages.length, session.status, lastMessage?.text, session.pendingPermission?.id, submitting]);
  ...
  return <div className="ai-panel">
    <div className={`ai-toolbar${autopilot ? " with-autopilot" : ""}`}>
      ...
      <select className="ai-agent-picker" aria-label="AI agent" title="Agent preset" value={selectedAgent} disabled={busy} onChange={(event) => onAgentChange(event.target.value)}><option value="">No agent</option>{(["global", "local", "workspace"] as const).map((scope) => { const scoped = agents.filter((agent) => agent.scope === scope); return scoped.length ? <optgroup key={scope} label={scope === "workspace" ? "Workspace" : scope[0]!.toUpperCase() + scope.slice(1)}>{scoped.map((agent) => <option key={`${agent.scope}:${agent.name}`} value={`${agent.scope}:${agent.name}`}>{agent.agent.name}</option>)}</optgroup> : null; })}</select>
      ...
      {running ? <button className="ai-interrupt" title={`Stop ${providerName}`} onClick={onInterrupt}><Square size={12} fill="currentColor" /></button> : <button className={sessionsOpen ? "active" : ""} title="Manage sessions" aria-expanded={sessionsOpen} disabled={submitting} onClick={() => { setSettingsOpen(false); setSessionsOpen((open) => !open); }}><MessageSquare size={14} /></button>}
    </div>
    {sessionsOpen && <section className="ai-sessions">
      <header><strong>Sessions</strong><button disabled={submitting} onClick={() => { setSessionsOpen(false); onNewSession(); }}><Plus size={13} /> New</button></header>
      <div>{sessions.filter((item) => item.id).map((item, index) => <div className="ai-session-row" key={item.id ?? index}><button className="ai-session-select" disabled={submitting} onClick={() => { setSessionsOpen(false); onSwitchSession(item); }}>{item.id === session.id && <Check size={13} />}<span><strong>{sessionTitle(item)}</strong><small>{sessionDate(item)}</small></span></button><button className="ai-session-remove" title="Remove session" disabled={submitting || sessions.length === 1} onClick={() => onRemoveSession(item)}><Trash2 size={13} /></button></div>)}</div>
    </section>}
