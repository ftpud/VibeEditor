...
export function AiPanel({ provider, providers, session, sessions, models, usage, attachments, onProviderChange, onConfigurationChange, onAttachmentsChange, onSend, onSteer, onInterrupt, onNewSession, onSwitchSession, onRemoveSession, onResolvePermission }: { provider: AiProvider; providers: AiProviderDescriptor[]; session: AiSession; sessions: AiSession[]; models: AiModel[]; usage?: AiUsage; attachments: AiAttachment[]; onProviderChange(provider: AiProvider): void; onConfigurationChange(configuration: AiConfiguration): void; onAttachmentsChange(attachments: AiAttachment[]): void; onSend(prompt: string, configuration: AiConfiguration, attachments: AiAttachment[]): Promise<void>; onSteer(prompt: string): Promise<void>; onInterrupt(): void; onNewSession(): void; onSwitchSession(session: AiSession): void; onRemoveSession(session: AiSession): void; onResolvePermission(requestId: string, optionId?: string): void }) {
  ...
  return <div className="ai-panel">
    ...
    {(reasoningLevels.length > 0 || autopilotOption) && <div className="ai-quick-settings">
      {reasoningLevels.length > 0 && <label className="ai-quick-setting" title={selectedModel?.reasoningDescriptions?.[reasoning] ?? "Higher effort can improve difficult tasks, but usually takes longer."}><span>Effort</span><select aria-label="Reasoning effort" value={reasoningLevels.includes(reasoning) ? reasoning : (selectedModel?.defaultReasoning ?? reasoningLevels[0])} disabled={running} onChange={(event) => changeReasoning(event.target.value)}>{reasoningLevels.map((level) => <option key={level} value={level} title={selectedModel?.reasoningDescriptions?.[level]}>{level}</option>)}</select></label>}
      {autopilotOption && <label className="ai-quick-setting ai-quick-toggle" title={autopilotOption.description}><span>{autopilotOption.name}</span>{autopilotOption.type === "boolean"
        ? <span className="ai-switch"><input type="checkbox" aria-label={autopilotOption.name} disabled={running} checked={Boolean(configuration[autopilotOption.id] ?? autopilotOption.defaultValue)} onChange={(event) => updateConfiguration({ [autopilotOption.id]: event.target.checked })} /><span /></span>
        : <select aria-label={autopilotOption.name} disabled={running} value={selectValue(autopilotOption, configuration[autopilotOption.id])} onChange={(event) => updateConfiguration({ [autopilotOption.id]: event.target.value })}>{autopilotOption.choices?.map((choice) => <option key={choice.value} value={choice.value} title={choice.description}>{choice.name}</option>)}</select>}</label>}
    </div>}
