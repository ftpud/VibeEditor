...
export function provisionCommand(settings: RepositorySettings, remoteNodeEnvironment: string, repair = false): string {
  ...
  const repairArtifacts = repair ? `echo 'VIBE_STAGE:repair'; rm -rf ~/.vibe/node_modules ~/.vibe-build ~/.vibe/packages/acp/dist ~/.vibe/packages/protocol/dist ~/.vibe/packages/core/dist ~/.vibe/packages/desktop/dist-electron ~/.vibe/packages/desktop/dist-renderer;` : "";
  return `set -e; echo 'VIBE_STAGE:checkout'; ${remoteNodeEnvironment}; if [ -d ~/.vibe/.git ]; then ${update} else rm -rf ~/.vibe; git clone --branch ${quotedBranch} --single-branch ${quotedRepository} ~/.vibe; fi; cd ~/.vibe; ${repairArtifacts} head=$(git rev-parse HEAD); rebuilt=0; if [ ! -f ~/.vibe-build ] || [ "$(cat ~/.vibe-build)" != "$head" ] || [ ! -f packages/core/dist/index.js ] || [ ! -f packages/desktop/dist-electron/main.js ] || [ ! -f packages/desktop/dist-renderer/index.html ] || [ ! -d node_modules ] || ! node -e "require('node-pty')" >/dev/null 2>&1; then echo 'VIBE_STAGE:install'; VIBE_SKIP_JDTLS=1 npm install; if ! node -e "require('node-pty')" >/dev/null 2>&1; then echo 'VIBE_STAGE:repair-node-pty'; if ! npm rebuild node-pty; then echo "node-pty could not be built. Install a C/C++ compiler, make, and Python 3 on the SSH host, then try again." >&2; exit 1; fi; fi; node -e "require('node-pty')" >/dev/null 2>&1 || { echo "node-pty is still unavailable after rebuilding it." >&2; exit 1; }; echo 'VIBE_STAGE:build'; rm -rf packages/acp/dist packages/protocol/dist packages/core/dist packages/desktop/dist-electron packages/desktop/dist-renderer; npm run build -w @remote-ide/acp; npm run build -w @remote-ide/protocol; npm run build -w @remote-ide/core; npm run build -w @remote-ide/desktop; printf '%s' "$head" > ~/.vibe-build; rebuilt=1; fi; printf '\nVIBE_RESULT:%s:%s\n' "$head" "$rebuilt"`;
}
