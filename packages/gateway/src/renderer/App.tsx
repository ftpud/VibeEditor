...
import { Cable, Check, CircleStop, Copy, HardDrive, Laptop, Network, Pencil, Play, Plus, RefreshCw, Server, Settings, Trash2, X } from "lucide-react";
...
export function ConnectionDialog({ value, onClose, onSave }: { value: Partial<GatewayConnection> & { password: string; passphrase: string }; onClose(): void; onSave(value: Partial<GatewayConnection> & { name: string; host: string; port: number; username: string; authenticationMethod: "password" | "privateKey" | "agent"; password?: string; passphrase?: string }): Promise<void> }) {
