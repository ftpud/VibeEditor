...
export function App() {
  ...
  return <div className="gateway-shell">
    <aside>
      <header><div><Network size={17} /><strong>Connections</strong></div><button title="Add connection" aria-label="Add connection" onClick={() => { clearError(); setConnectionDialog({ kind: "ssh", port: 22, authenticationMethod: "password", password: "", passphrase: "" }); }}><Plus size={15} /></button></header>
      <div className="connection-list" aria-label="Connections">{state.connections.map((item) => { const current = connectionRuntimes[item.id] ?? { status: "unknown" }; const local = item.kind === "local"; return <button key={item.id} className={item.id === selected ? "selected" : ""} aria-pressed={item.id === selected} aria-label={`${item.name}, ${local ? "local Mac" : `${connectionHealthName(current.status)}, ${item.username} at ${item.host}, port ${item.port}`}`} onClick={() => { clearError(); setSelected(item.id); }}><div className={`status-dot connection-${current.status}`} aria-hidden="true" />{local ? <HardDrive size={16} /> : <Server size={16} />}<span><strong>{item.name}</strong><small>{local ? "This Mac" : `${item.username}@${item.host}:${item.port}`}</small></span></button>; })}</div>
    </aside>
    <main>
      <div className="topbar"><div><HardDrive size={18} /><span>Vibe Gateway</span></div><div className="connection-actions">{connection && <><span>{connection.name}</span><span className={`connection-health ${connectionRuntime!.status}`} title={connectionRuntime!.message} role="status" aria-live="polite">{connection.kind === "local" ? "Local" : connectionHealthName(connectionRuntime!.status)}{connection.kind !== "local" && connectionRuntime!.latencyMs !== undefined && ` · ${connectionRuntime!.latencyMs} ms`}</span><button title="Refresh connection and Core status" aria-label="Refresh connection and Core status" onClick={() => void refreshConnection(connection.id)}><RefreshCw size={14} /></button><DiagnosticsCopyButton onCopy={() => window.gateway.copyDiagnostics(connection.id)} onFailure={(reason) => showError(reason, "Could not copy diagnostics: ")} /><button title="Edit connection" aria-label={`Edit connection ${connection.name}`} onClick={() => { clearError(); setConnectionDialog({ ...connection, password: "", passphrase: "" }); }}><Pencil size={14} /></button><button title="Delete connection" aria-label={`Delete connection ${connection.name}`} onClick={() => void removeConnection(connection)}><Trash2 size={14} /></button></>}<button title="Gateway repository settings" aria-label="Gateway repository settings" onClick={() => { if (repository) { clearError(); setRepositoryDialog(repository); } }}><Settings size={15} /></button></div></div>
      {!bridgeReady ? <div className="empty"><Network size={38} /><strong>Gateway failed to initialize</strong><span>Close the application and run <code>npm run gateway</code> again.</span></div> : !connection ? <div className="empty"><Network size={38} /><strong>No connections</strong><span>Add this Mac or an SSH host to manage Vibe Editor workspaces.</span><button onClick={() => setConnectionDialog({ kind: "ssh", port: 22, authenticationMethod: "password", password: "", passphrase: "" })}><Plus size={15} /> Add connection</button></div> : <section className="workspace-view">
        <header><div><h1>{connection.kind === "local" ? "Local workspaces" : "Remote workspaces"}</h1><p>{connection.kind === "local" ? "Projects stored on this Mac" : `${connection.username}@${connection.host}`}</p></div><div className="workspace-header-actions">{connection.kind !== "local" && <button className="secondary" onClick={() => { clearError(); setDiscoverWorkspaceOnOpen(true); setWorkspaceDialog({ connectionId: connection.id, remotePort: 7331 }); }}><Search size={15} /> Discover workspaces</button>}<button onClick={() => { clearError(); setDiscoverWorkspaceOnOpen(false); setWorkspaceDialog({ connectionId: connection.id, remotePort: 7331 }); }}><Plus size={15} /> Add workspace</button></div></header>
        <div className="workspace-list">{workspaces.map((workspace) => { const current = runtimes[workspace.id] ?? { status: "idle", message: "Not running" }; const busy = current.status === "working"; const serverRunning = current.status === "server" || current.status === "client"; return <article key={workspace.id}>
          <div className="workspace-info"><div className={`status-dot ${current.status}`} aria-hidden="true" /><div><h2>{workspace.name}</h2><code>{workspace.directory}</code><span>{connection.kind === "local" ? "Core" : "Remote"} port {workspace.remotePort}</span></div><div className="status" role="status" aria-live="polite"><strong>{statusName(current.status)}</strong><small>{current.message}</small>{current.logs?.length ? <details><summary>Provisioning log</summary><pre>{current.logs.join("\n")}</pre></details> : null}</div></div>
        ...
        {connection.kind !== "local" && <section className="tunnel-section">
    ...
    {bridgeReady && workspaceDialog && connection && <WorkspaceDialog value={workspaceDialog} connectionId={connection.id} local={connection.kind === "local"} discoverOnOpen={discoverWorkspaceOnOpen} configuredDirectories={workspaces.map((item) => item.directory)} onClose={() => setWorkspaceDialog(undefined)} onFailure={showError} onSave={async (value) => { setState(await window.gateway.saveWorkspace(value)); setWorkspaceDialog(undefined); clearError(); }} />}
...
export function ConnectionDialog({ value, onClose, onSave, onFailure = () => undefined }: { value: Partial<GatewayConnection> & { password: string; passphrase: string }; onClose(): void; onSave(value: Partial<GatewayConnection> & { kind?: "ssh" | "local"; name: string; host: string; port: number; username: string; authenticationMethod: "password" | "privateKey" | "agent"; password?: string; passphrase?: string }): Promise<void>; onFailure?(reason: unknown): void }) {
  ...
  const dialogName = form.id ? `Edit ${local ? "local" : "SSH"} connection` : "New connection";
  useDialogKeyboard(onClose, dialogName);
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { await onSave({ ...form, kind: local ? "local" : "ssh", name: form.name!.trim(), host: form.host?.trim() ?? "", port: Number(form.port ?? 0), username: form.username?.trim() ?? "", authenticationMethod: form.authenticationMethod ?? "password", password: form.password, passphrase: form.passphrase }); } catch (reason) { onFailure(reason); setSaving(false); } };
...
export function WorkspaceDialog({ value, connectionId, local = false, discoverOnOpen = false, configuredDirectories = [], onClose, onSave, onFailure }: { value: Partial<GatewayWorkspace>; connectionId: string; local?: boolean; discoverOnOpen?: boolean; configuredDirectories?: string[]; onClose(): void; onSave(value: Partial<GatewayWorkspace> & { connectionId: string; name: string; directory: string; remotePort: number }): Promise<void>; onFailure(reason: unknown): void }) {
  ...
  useDialogKeyboard(onClose, form.id ? `Edit ${local ? "local" : "remote"} workspace` : `New ${local ? "local" : "remote"} workspace`);
  ...
  const browse = async () => { try { const directory = await window.gateway.pickWorkspaceDirectory(); if (directory) chooseDirectory(directory); } catch (reason) { onFailure(reason); } };
